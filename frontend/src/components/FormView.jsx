...
export default function FormView({ onReportAdded }) {
  ...
  const appendTag = (major, minor) => {
    ...
    const prevText = activeField === 'thisWeekTask' ? thisWeekTask : nextWeekTask;
    const isFirst = !prevText || prevText.trim().length === 0;
    ...
    if (activeField === 'thisWeekTask') {
      setThisWeekTask(prev => (prev ? prev.trimEnd() : '') + textToAppend);
    } else {
      setNextWeekTask(prev => (prev ? prev.trimEnd() : '') + textToAppend);
    }
    
    setLastState(prev => ({ 
      ...prev, 
      [activeField]: { major, minor } 
    }));
  };
